...
const FormField = ({ label, required, error, children }) => (
  <div className="quote-form-field">
    ...
    <AnimatePresence>
      {error && (
        <motion.p initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} className="quote-form-error">
          {error}
        </motion.p>
      )}
    </AnimatePresence>
...
const ServiceCheckbox = ({ service, checked, onChange }) => (
  <label className={`quote-service-checkbox ${checked ? "quote-service-checkbox-checked" : ""}`}>
...
const OptionButton = ({ option, selected, onChange }) => (
  <label className={`quote-option-button ${selected ? "quote-option-button-selected" : ""}`}>
    <input type="radio" checked={selected} onChange={onChange} className="sr-only" />
    <span className="quote-option-label">{option.label}</span>
  </label>
);

export default function QuoteRequestFormClean({ preselectedService = null, className = "" }) {
  ...
  const validateAll = () => ["firstName", "lastName", "email", "message"].every((field) => !validateField(field));
      ...
      <form onSubmit={handleSubmit} className="quote-form" noValidate>
        <AnimatePresence>
          {submitStatus === "success" && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="quote-form-success">
        ...
        <AnimatePresence>
          {submitStatus === "error" && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="quote-form-error-box">
          ...
          <>
            <div className="quote-form-section">
              ...
              <div className="quote-form-grid-2">
                <FormField label="Etunimi" required error={touched.firstName && errors.firstName}>
                  <input type="text" value={formData.firstName} onChange={(e) => handleChange("firstName", e.target.value)} onBlur={() => { setTouched((prev) => ({ ...prev, firstName: true })); validateField("firstName"); }} className={`quote-form-input ${touched.firstName && errors.firstName ? "quote-form-input-error" : ""}`} placeholder="Matti" />
                </FormField>
                <FormField label="Sukunimi" required error={touched.lastName && errors.lastName}>
                  <input type="text" value={formData.lastName} onChange={(e) => handleChange("lastName", e.target.value)} onBlur={() => { setTouched((prev) => ({ ...prev, lastName: true })); validateField("lastName"); }} className={`quote-form-input ${touched.lastName && errors.lastName ? "quote-form-input-error" : ""}`} placeholder="Meikäläinen" />
                </FormField>
              </div>
              <div className="quote-form-grid-2">
                <FormField label="Sähköposti" required error={touched.email && errors.email}>
                  <input type="email" value={formData.email} onChange={(e) => handleChange("email", e.target.value)} onBlur={() => { setTouched((prev) => ({ ...prev, email: true })); validateField("email"); }} className={`quote-form-input ${touched.email && errors.email ? "quote-form-input-error" : ""}`} placeholder="matti@esimerkki.fi" />
                </FormField>
                <FormField label="Puhelin" required error={touched.phone && errors.phone}>
                  <input type="tel" value={formData.phone} onChange={(e) => handleChange("phone", e.target.value)} onBlur={() => { setTouched((prev) => ({ ...prev, phone: true })); validateField("phone"); }} className={`quote-form-input ${touched.phone && errors.phone ? "quote-form-input-error" : ""}`} placeholder="+358 40 123 4567" />
                </FormField>
              </div>
            </div>
              ...
              <FormField label="Mitä palveluita tarvitset?">
                <div className="quote-services-grid">
                  {SERVICE_OPTIONS.map((service) => (
                    <ServiceCheckbox key={service.id} service={service} checked={formData.services.includes(service.id)} onChange={() => handleServiceToggle(service.id)} />
                  ))}
                </div>
              </FormField>
              <FormField label="Kohde">
                <div className="quote-options-grid">
                  {PROPERTY_TYPES.filter((opt) => opt.value !== "").map((opt) => (
                    <OptionButton key={opt.value} option={opt} selected={formData.propertyType === opt.value} onChange={() => handleChange("propertyType", opt.value)} />
                  ))}
                </div>
              </FormField>
              <FormField label="Pinta-ala">
                <div className="quote-options-grid">
                  {AREA_SIZES.filter((opt) => opt.value !== "").map((opt) => (
                    <OptionButton key={opt.value} option={opt} selected={formData.areaSize === opt.value} onChange={() => handleChange("areaSize", opt.value)} />
                  ))}
                </div>
              </FormField>
              <div className="quote-form-grid-2">
                <FormField label="Sijainti">
                  <input type="text" value={formData.location} onChange={(e) => handleChange("location", e.target.value)} className="quote-form-input" placeholder="Esim. Helsinki, Espoo" />
                </FormField>
                <FormField label="Aikataulu">
                  <div className="quote-options-grid-compact">
                    {TIMELINES.filter((opt) => opt.value !== "").map((opt) => (
                      <OptionButton key={opt.value} option={opt} selected={formData.timeline === opt.value} onChange={() => handleChange("timeline", opt.value)} />
                    ))}
                  </div>
                </FormField>
              </div>
            ...
            <div className="quote-form-section">
              ...
              <FormField label="Kerro projektistasi" required error={touched.message && errors.message}>
                <textarea value={formData.message} onChange={(e) => handleChange("message", e.target.value)} onBlur={() => { setTouched((prev) => ({ ...prev, message: true })); validateField("message"); }} rows={4} className={`quote-form-textarea ${touched.message && errors.message ? "quote-form-input-error" : ""}`} placeholder="Kuvaile projektisi: mitä tehdään, missä, milloin, ja muut tärkeät tiedot..." />
              </FormField>
            </div>

            <button type="submit" disabled={isSubmitting} className="quote-form-submit">
              {isSubmitting ? (
                <>
